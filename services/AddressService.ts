import { searchMahalleler } from '../data/dataService';
...
class AddressService {
  ...
  async search(query: string, limit: number = 10): Promise<SearchResult[]> {
    ...
    try {
      ...
      const mahalleResults = searchMahalleler(query, Math.ceil(limit * 0.5));
      const mahalleResultsNormalized = queryNormalized !== queryLower ? 
        searchMahalleler(queryNormalized, Math.ceil(limit * 0.5)) : [];
      
      console.log(`🏘️ Mahalle sonuçları (orijinal): ${mahalleResults.length}`);
      console.log(`🏘️ Mahalle sonuçları (normalize): ${mahalleResultsNormalized.length}`);
      
      // Birleştir ve dublicate'leri temizle
      const allMahalleResults = [...mahalleResults, ...mahalleResultsNormalized];
      const uniqueMahalleResults = allMahalleResults.filter((result, index, self) =>
        index === self.findIndex(r => r.mahalle.mahalle_id === result.mahalle.mahalle_id)
      );

      console.log(`🏘️ Benzersiz mahalle sonuçları: ${uniqueMahalleResults.length}`);

      uniqueMahalleResults.forEach(result => {
        const score = Math.max(
          this.calculateScore(queryLower, result.mahalle.mahalle_adi.toLowerCase()),
          this.calculateScore(queryNormalized, this.normalizeText(result.mahalle.mahalle_adi))
        );
        results.push({
          id: `mahalle-${result.mahalle.mahalle_id}`,
          name: result.mahalle.mahalle_adi,
          type: 'Mahalle',
          fullAddress: result.fullAddress,
          score,
          quarterId: result.mahalle.mahalle_id,
          districtId: result.mahalle.ilce_id,
          cityId: result.sehirId
        });
      });
      ...
      console.log(`🛣️ Cadde sonuçları (orijinal): ${streetMatches.length}`);
      console.log(`🛣️ Cadde sonuçları (normalize): ${streetMatchesNormalized.length}`);
